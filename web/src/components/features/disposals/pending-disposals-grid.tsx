...
export function PendingDisposalsGrid({ initialData, onRowClick }: PendingDisposalsGridProps) {
  ...
  const columns = useMemo<ColumnDef<PendingDisposalRow>[]>(() => [
    ...
    { accessorKey: 'assetName', header: 'Device Name', cell: ({ row }) => row.original.assetName || '-' },
    ...
    { accessorKey: 'reason', header: 'Reason' },
    {
      ...
      header: 'Days Pending',
      cell: ({ row }) => {
        const days = Math.floor(Math.abs(new Date().getTime() - new Date(row.original.requestedAt).getTime()) / (1000 * 60 * 60 * 24));
        return (
          <span className="inline-flex h-6 items-center rounded-full border border-amber-300 bg-amber-50/50 px-2.5 text-[11px] font-semibold text-amber-600">
    ...
    <div className="flex flex-col flex-1 min-h-0 gap-4">
      ...
      <div className="min-h-0 flex-1">
        <DataTable<PendingDisposalRow, unknown>
          ...
          onRowClick={(row) => onRowClick(row)} // Pass the full row data up
        />
      </div>
    </div>
  );
