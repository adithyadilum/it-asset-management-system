...
export function StepPreview({ state, dispatch }: StepPreviewProps) {
  ...
  const getStageBadgeColor = (stage: string) => {
    switch (stage) {
      case 'STRUCTURAL': return 'bg-muted text-muted-foreground border-border';
      case 'TYPE': return 'bg-destructive/10 text-destructive border-destructive/20';
      case 'REFERENTIAL': return 'bg-primary/10 text-primary border-primary/20';
      ...
      <div className="flex flex-col flex-1 px-8 py-6 gap-6 min-h-0">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="rounded-xl border border-success/30 bg-success/10 p-4 transition-shadow hover:shadow-sm">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm font-medium text-success/80">Ready to import</p>
              </div>
            </div>
          </div>
          ...
          <div className={cn(
            ...
            <div className="flex items-start gap-3">
              <CircleX className={cn("size-5 mt-0.5", errorRows.length > 0 ? "text-destructive" : "text-muted-foreground")} />
              <div>
                <p className={cn("text-2xl font-bold", errorRows.length > 0 ? "text-destructive" : "text-foreground")}>
                  {errorRows.length}
                </p>
                <p className={cn("text-sm font-medium", errorRows.length > 0 ? "text-destructive/80" : "text-muted-foreground")}>
                  Will be skipped
                </p>
              </div>
            </div>
        ...
        {errorRows.length > 0 && (
          <div className="rounded-lg border overflow-hidden flex flex-col min-h-0 min-w-0 flex-1">
            <div className="bg-muted px-4 py-2 border-b grid grid-cols-[60px_1fr_120px_120px_1fr] gap-3">
              <div className="text-xs font-semibold text-muted-foreground">Row #</div>
              <div className="text-xs font-semibold text-muted-foreground">Asset Name</div>
            ...
            <ScrollArea className="h-full flex-1">
              <div className="divide-y divide-border">
                {errorRows.map((err, i) => (
                  <div key={i} className="px-4 py-2.5 grid grid-cols-[60px_1fr_120px_120px_1fr] gap-3 hover:bg-muted/50">
                    <div className="text-xs text-muted-foreground">{err.rowNumber}</div>
                    <div className="text-xs text-foreground font-medium truncate" title={err.assetName || '-'}>
                      {err.assetName || '-'}
                    </div>
                    <div className="text-xs text-foreground truncate" title={err.errorField || '-'}>
                      {err.errorField || '-'}
                    </div>
                    <div>
                      <span className={cn(
                    ...
                    <div className="text-xs text-muted-foreground truncate" title={err.errorMessage}>
                      {err.errorMessage}
                    </div>
                  </div>
                ))}
