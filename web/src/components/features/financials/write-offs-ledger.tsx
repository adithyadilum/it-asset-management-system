...
export function WriteOffsLedger({ initialData }: WriteOffsLedgerProps) {
  ...
  const columns: ColumnDef<WriteOffsLedgerRecord>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => <span className="text-slate-600">{formatCurrency(row.original.bookValue)}</span>,
    },
    {
      accessorKey: "salvageValue",
      header: "Salvage Value",
      cell: ({ row }) => <span className="text-slate-600">{formatCurrency(row.original.salvageValue)}</span>,
    },
  ];

  return (
    <div className="flex flex-col gap-4"> {/* Removed h-full to allow natural scrolling */}
      {/* Toolbar */}
      ...
      <DataTable
        ...
        initialPageSize={16}
        enableSelection={false} // Checkboxes disabled
