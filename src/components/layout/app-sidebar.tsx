  ...
  if (href === '/assets') {
    ...
    return (
      ...
      (pathname.startsWith('/assets/') &&
        !excludedSubPaths.some((sub) => pathname.startsWith(sub)))
    );
  }
      ...
      {items.map((item) => {
        ...
        return (
          <Collapsible
            key={item.label}
            defaultOpen={
              isItemActive || children.some((child) => child.isActive)
            }
    ...
    >
      ...
      <SidebarContent className="gap-0 overflow-x-hidden overflow-y-auto px-0 py-0 group-data-[collapsible=icon]:overflow-y-auto">
        <div className="px-2 pt-2">
          ...
          {!collapsed && visibleAssets.length > 0 ? (
            <SectionLabel title="ASSETS" />
          ) : null}
          <NavGroup
            items={visibleAssets}
            collapsed={collapsed}
