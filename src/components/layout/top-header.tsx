...
export function TopHeader({ user }: TopHeaderProps) {
  ...
  return (
    <header className="grid h-14 w-full grid-cols-2 md:grid-cols-3 items-center gap-4 rounded-none border-none bg-background md:rounded-lg md:bg-muted md:px-2">
      {' '}
      {/* Left Column: Mobile Logo / Desktop Breadcrumb */}
      ...
      <div className="hidden min-w-0 items-center gap-2 px-2 md:flex">
        ...
        <div className="flex items-center px-2">
          <Separator orientation="vertical" className="h-4.25 w-px bg-muted" />
        </div>
        ...
        <Breadcrumb>
          <BreadcrumbList className="min-w-0 flex-nowrap gap-1.5 overflow-hidden text-inherit">
            {breadcrumbs.length === 0 ? (
              <BreadcrumbItem>
                <BreadcrumbPage
                  className={`${sidebarDefaultTextClass} whitespace-nowrap text-foreground`}
                >
                  Dashboard
                </BreadcrumbPage>
              </BreadcrumbItem>
              ...
              breadcrumbs.map((breadcrumb, index) => {
                ...
                return [
                  <BreadcrumbItem key={`${breadcrumb.href}-item`}>
                    {isLast ? (
                      <BreadcrumbPage
                        className={`${sidebarDefaultTextClass} truncate whitespace-nowrap text-foreground`}
                      >
                        {breadcrumb.label}
                      </BreadcrumbPage>
                    ) : (
                  ...
                  <DropdownMenuPortal>
                    <DropdownMenuSubContent>
                      <DropdownMenuItem
                        ...
                        className="h-9 justify-start rounded-lg text-xs cursor-pointer focus:bg-muted"
                      >
