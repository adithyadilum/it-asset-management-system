...
describe('InteractiveStatusBadge', () => {
  ...
  it('submits form and calls server action with justification', async () => {
    ...
    (manualStatusOverrideAction as any).mockResolvedValue({ success: true, message: 'Updated' });
  ...
  it('shows error toast on server action failure', async () => {
    const user = userEvent.setup();
    (manualStatusOverrideAction as any).mockResolvedValue({ success: false, message: 'Override failed' });
