...
describe('TableEmptyState', () => {
  it('renders default text when no props are provided', () => {
    ...
    expect(
      screen.getByText('There is nothing to show in this table yet.')
    ).toBeInTheDocument();
  });
  ...
  it('renders action link when action.href is provided', () => {
    render(<TableEmptyState action={{ label: 'Go to Home', href: '/home' }} />);
  ...
  it('renders action button and triggers onClick when action.href is omitted', async () => {
    ...
    render(
      <TableEmptyState action={{ label: 'Click Me', onClick: onClickMock }} />
    );
