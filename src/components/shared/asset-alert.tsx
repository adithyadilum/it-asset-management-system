...
const ALERT_STYLE_MAP: Record<AssetAlertVariant, AlertStyleConfig> = {
  reminder: {
    container:
      'border-alert-reminder-border bg-alert-reminder-bg text-alert-reminder-text',
    button:
      'bg-alert-reminder-btn text-primary-foreground hover:bg-alert-reminder-btn-hover',
  },
  'return-overdue': {
    container:
      'border-alert-destructive-border bg-alert-destructive-bg text-alert-destructive-text',
    button:
      'bg-alert-destructive-btn text-primary-foreground hover:bg-alert-destructive-btn-hover',
  },
  notice: {
    container:
      'border-alert-destructive-border bg-alert-destructive-bg text-alert-destructive-text',
  ...
  'repair-completed': {
    container:
      'border-alert-success-border bg-alert-success-bg text-alert-success-text',
  },
  'action-required': {
      ...
      {actionNode ? (
        ...
        actionHref ? (
          <Button
            asChild
            size="sm"
            className={cn('shrink-0 rounded-lg px-4', styles.button)}
          >
