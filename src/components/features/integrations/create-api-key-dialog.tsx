      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-120 p-0">
          <DialogHeader className="p-6">
            <DialogTitle
              className={`flex items-center gap-2 text-foreground ${TYPOGRAPHY_CLASSNAMES.textLgSemiBold}`}
            >
          ...
          <div className="p-6 pt-0">
            <div className="grid gap-3">
              <div className="grid gap-2">
                <Label className={TYPOGRAPHY_CLASSNAMES.textSmMedium}>
                  Name
                </Label>
                <Input
                  value={name}
              ...
              <div>
                <Label className={`mb-2 ${TYPOGRAPHY_CLASSNAMES.textSmMedium}`}>
                  Scopes
                </Label>
                <div className="grid gap-2">
                  {Object.entries(API_KEY_SCOPE_GROUPS).map(
                    ([group, items]) => (
                      <div
                        key={group}
                      ...
                      >
                        <div
                          className={`mb-2 text-foreground ${TYPOGRAPHY_CLASSNAMES.textSmMedium}`}
                        >
                          {group}
                        </div>
                        <div className="grid gap-2">
                          {items.map((it) => (
                            ...
                            >
                              ...
                              <div>
                                <div
                                  className={`text-foreground ${TYPOGRAPHY_CLASSNAMES.textSmMedium}`}
                                >
                                  {it.label}
                                </div>
                                <div
                                  className={`text-muted-foreground ${TYPOGRAPHY_CLASSNAMES.textXsRegular}`}
                                >
                                  {it.description}
                                </div>
                              </div>
