...
export function SandboxUiPlaygroundClient() {
  ...
  const tabbedPanelTabs = [
    {
      ...
      content: (
        <div className="space-y-3">
          ...
          >
            Maria is a full-time employee in IT Operations with direct access to
            asset assignment and maintenance workflows.
          </p>
          <div className="rounded-md bg-muted p-3">
            <dl className="space-y-2">
              <div className="flex items-center justify-between gap-3">
                <dt
                  className={`${TYPOGRAPHY_CLASSNAMES.textXsRegular} text-foreground`}
                >
                  Department
                </dt>
                <dd
  ...
  return (
    <div className="px-4 pb-4 md:px-6 md:pb-6">
      <div className="flex min-h-0 flex-1 flex-col gap-6 rounded-2xl border border-dashed border-border bg-muted p-4 sm:p-6">
        <div className="px-1">
          <h1
            className={`${TYPOGRAPHY_CLASSNAMES.text2xlSemiBold} text-foreground`}
          >
            TIQRI Design Sandbox
          </h1>
          <p
            className={`${TYPOGRAPHY_CLASSNAMES.textSmRegular} text-muted-foreground`}
          >
            Isolate and test shared components here.
          </p>
        </div>
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          >
            Slide Panel Archetype Triggers
          </h2>
          <div className="flex flex-wrap items-center gap-2">
            <Button variant="outline" onClick={() => openPanel('form')}>
              Open FormPanel
            </Button>
            <Button variant="outline" onClick={() => openPanel('detail')}>
              Open DetailPanel
            </Button>
            <Button variant="outline" onClick={() => openPanel('tabbed')}>
              Open TabbedPanel
            </Button>
        ...
        <div className="grid min-h-0 flex-1 grid-cols-[repeat(auto-fit,minmax(220px,1fr))] gap-3 sm:gap-4">
          <div className="rounded-xl bg-card p-4">
            ...
            <p
              className={`${TYPOGRAPHY_CLASSNAMES.textSmRegular} text-muted-foreground`}
            >
              Workhorse for create/edit flows with built-in form wrapper, submit
              loading state, and standardized Cancel/Submit actions.
            </p>
          </div>
          ...
          <div className="rounded-xl bg-card p-4">
            ...
            <p
              className={`${TYPOGRAPHY_CLASSNAMES.textSmRegular} text-muted-foreground`}
            >
              Read-only viewer optimized for key-value data using description
              lists and optional badges in the header.
            </p>
          </div>
          ...
          <div className="rounded-xl bg-card p-4">
            ...
            <p
              className={`${TYPOGRAPHY_CLASSNAMES.textSmRegular} text-muted-foreground`}
            >
              Complex entity viewer with tab navigation below the header and
              stable scroll behavior while switching tabs.
            </p>
          </div>
        </div>
          ...
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <div>
              <p
                className={`${TYPOGRAPHY_CLASSNAMES.textSmMedium} text-foreground`}
              >
                Status Badge Testing Suite
              </p>
              <p
                className={`${TYPOGRAPHY_CLASSNAMES.textSmRegular} text-muted-foreground`}
              >
                Validate all known badge states, icon toggle behavior, and
