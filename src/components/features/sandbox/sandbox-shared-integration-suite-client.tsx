    ...
    () => [
      {
        ...
        content: (
          <p className="text-sm text-muted-foreground">
            Shared components are rendered together to validate layout,
            behavior, and visual consistency.
          </p>
        ),
      },
      {
        ...
        content: (
          <p className="text-sm text-muted-foreground">
            Current delete-candidate count:{' '}
    ...
    <section className="px-4 pb-4 md:px-6 md:pb-6">
      <div className="rounded-xl border border-dashed border-border bg-muted p-4 md:p-6">
        <h2 className="text-lg font-semibold text-foreground">
          Shared Components Integration Suite
        </h2>
        <p className="mt-1 text-sm text-muted-foreground">
          Composite sandbox to validate all shared component building blocks in
          one route.
        </p>
        ...
        <div className="mt-5 grid gap-4 lg:grid-cols-2">
          ...
          <div className="rounded-lg bg-card p-4 lg:col-span-2">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Reusable alert banner for reminder, return-overdue, and notice
              workflows.
            </p>
            ...
            <div className="mt-4 grid gap-4 xl:grid-cols-3">
              {assetCards.length > 0 ? (
                assetCards.map((asset) => {
                  const assetId =
                    asset.details.find((d) => d.label === 'Asset ID')?.value ??
                    '-';
                  const assignedDate =
                    asset.details.find((d) => d.label === 'Assigned')?.value ??
                    '-';
                ...
                <div className="rounded-lg border border-dashed border-border bg-background p-4 text-sm text-muted-foreground xl:col-span-3">
                  No active assignments were found for this employee in the
                  database.
                </div>
              )}
            </div>
          ...
          <div className="rounded-lg bg-card p-4">
            ...
            <div className="mt-3 flex items-center gap-3">
              <DestructiveConfirmationDialog
                ...
                errorMessage={
                  items.length === 0 ? 'No items left to delete.' : undefined
                }
              />
