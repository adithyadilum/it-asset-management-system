...
const sandboxData: SandboxAssetRow[] = Array.from(
  { length: 38 },
  (_, index) => {
...
const sandboxSelectionActions: DataTableSelectionAction<SandboxAssetRow>[] = [
  {
    ...
    onClick: (selectedRows) =>
      tiqriToast.info(
        `Printing QR codes for ${selectedRows.length} selected assets.`
      ),
  },
  {
    id: 'assign',
    ...
    onClick: (selectedRows) =>
      tiqriToast.success(
        `Assignment started for ${selectedRows.length} selected assets.`
      ),
  },
  {
    id: 'dispose',
    ...
    onClick: (selectedRows) =>
      tiqriToast.warning(
        `Disposal workflow opened for ${selectedRows.length} selected assets.`
      ),
  },
];
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-4 p-4 md:p-6">
      ...
      <section className="rounded-lg border border-dashed border-chart-2 bg-background p-4 md:p-6">
        ...
        <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          ...
          <Button
            ...
            onClick={() =>
              tiqriToast.loading(
                'Sync in Progress: Validating and syncing records...'
              )
            }
          >
            loading
          ...
          <Button
            ...
            variant="outline"
            onClick={() =>
              tiqriToast.actionable(
                'Approval Needed: Confirm onboarding for this asset.',
                {
                  action: {
                    label: 'Approve',
                    onClick: () =>
                      tiqriToast.success(
                        'Approved: Asset onboarding has been confirmed.'
                      ),
                  },
                  cancel: {
                    label: 'Later',
          ...
          <Button
            ...
            variant="outline"
            onClick={() =>
              tiqriToast.warningAction(
                'Security Warning: Unusual login activity detected.',
                {
                  action: {
                    label: 'Review',
                    onClick: () =>
                      tiqriToast.info(
                        'Review Started: Security activity panel opened.'
                      ),
                  },
                }
              )
          ...
          <Button
            ...
            variant="outline"
            onClick={() =>
              tiqriToast.infoAction(
                'FYI: A newer policy revision is available.',
                {
                  action: {
                    label: 'Open',
                    onClick: () =>
                      tiqriToast.success(
                        'Opened: Policy revision is now visible.'
                      ),
                  },
                }
              )
      ...
      <section className="flex min-h-0 flex-1 flex-col rounded-lg border border-dashed border-chart-2 bg-background p-4 md:p-6">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Typed reusable DataTable demo using{' '}
          <code>ColumnDef&lt;SandboxAssetRow&gt;</code>.
        </p>
