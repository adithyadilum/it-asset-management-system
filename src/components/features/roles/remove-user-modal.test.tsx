...
describe('RemoveUserModal', () => {
  ...
  it('renders correctly when isOpen is true', () => {
    ...
    expect(
      screen.getByText('Remove User from IT Operations')
    ).toBeInTheDocument();
  ...
  it('displays error message when removal fails', async () => {
    (removeUserFromManagedRole as any).mockResolvedValue({
      success: false,
  ...
  it('displays default error message on catch block', async () => {
    (removeUserFromManagedRole as any).mockRejectedValue(
    ...
    await waitFor(() => {
      expect(
        screen.getByText('Failed to remove user from this role.')
      ).toBeInTheDocument();
    });
  });
});
