...
import { useState, useRef, useCallback, useMemo, useEffect, useActionState } from "react";
...
export function useRegistrationForm({
  ...
  const [currencyCode, setCurrencyCode] = useState(CURRENCY_OPTIONS[0]?.value ?? "");
  ...
  const handleInvoiceSelection = useCallback((files: FileList | null) => {
    ...
    if (selectedFile && !isInvoiceAttachmentFile(selectedFile)) {
      tiqriToast.error("Upload a supported document or image file for invoice attachment.");
  ...
  const [prevFilteredModelOptions, setPrevFilteredModelOptions] = useState(filteredModelOptions);
  if (filteredModelOptions !== prevFilteredModelOptions) {
    setPrevFilteredModelOptions(filteredModelOptions);
    if (modelId.length > 0) {
      const stillValidModel = filteredModelOptions.some((option) => option.value === modelId);
      if (!stillValidModel) {
        setModelId("");
      }
    }
  ...
  const [prevFilteredBrandOptions, setPrevFilteredBrandOptions] = useState(filteredBrandOptions);
  if (filteredBrandOptions !== prevFilteredBrandOptions) {
    setPrevFilteredBrandOptions(filteredBrandOptions);
    if (brandId.length > 0) {
      const stillValidBrand = filteredBrandOptions.some((option) => option.value === brandId);
      if (!stillValidBrand) {
        setBrandId("");
      }
    }
  ...
  const selectedModelLabel = modelOptions.find((option) => option.value === modelId)?.label ?? "";
  ...
  const [prevConfigCondition, setPrevConfigCondition] = useState(config.defaultCondition);
