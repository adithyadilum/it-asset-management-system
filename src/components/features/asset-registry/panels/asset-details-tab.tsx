...
export function AssetDetailsTab({
      ...
      >
        ...
        <section className="rounded-lg border border-border/60 bg-card p-5 shadow-xs">
          <div className="space-y-4">
            ...
            <div className="flex justify-between items-center text-xs text-muted-foreground mt-2">
              <span>
                {Math.max(0, totalSeats - allocatedCount)} seats available
              </span>
              <span>
                {totalSeats > 0
                  ? Math.round((allocatedCount / totalSeats) * 100)
                  : 0}
                % used
              </span>
            </div>
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
          {softwareSections.map((section) => (
            <section
              key={section.title}
              className="rounded-lg border border-border/60 bg-card p-5 shadow-xs"
            >
              <h3
                className={cn(
                  TYPOGRAPHY_CLASSNAMES.textSmSemiBold,
                  'mb-4 text-sm text-foreground'
                )}
              >
                {section.title}
              </h3>
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        className={cn(
                          TYPOGRAPHY_CLASSNAMES.textSmMedium,
                          'text-right text-foreground min-w-0'
                        )}
                      >
                        {row.value || '-'}
                      </div>
  ...
  return (
    <div
      className={cn(
        'flex w-full flex-col items-start gap-4 text-sm text-foreground',
        className
      )}
    >
