...
export function EmployeeAlerts({ alerts }: EmployeeAlertsProps) {
  ...
  const [selectedAssignment, setSelectedAssignment] =
    useState<PendingAcceptanceItem | null>(null);
  ...
  const handleConfirmAccept = async () => {
    ...
    try {
      ...
      if (!res?.success)
        throw new Error(res?.error ?? 'Failed to accept assignment');
        ...
        <AssetAlert
          ...
          actionNode={
            <AcceptAssignmentDialog
              ...
              isOpen={
                selectedAssignment?.assignmentId === item.assignmentId &&
                isAcceptOpen
              }
