  ...
  const cardContent = (
    <Card
      className={cn(
        ...
        href &&
          'cursor-pointer hover:shadow-md hover:border-primary/30 hover:-translate-y-0.5 group/kpi-card'
      )}
      ...
      <CardHeader className="flex flex-row items-center justify-between p-4 pb-2 space-y-0">
        <CardTitle
          className={cn(
            ...
            'text-muted-foreground group-hover/kpi-card:text-foreground transition-colors duration-250'
          )}
        >
          {title}
        </CardTitle>
      ...
      <CardContent className="p-4 pt-1 flex flex-col gap-1 flex-1 justify-between">
        <div className="flex items-center gap-1.5 mt-1">
          {currencySymbol && (
            <Badge
              variant="secondary"
              className="px-1.5 py-0 border-border/50 text-[10px] font-bold tracking-wider uppercase text-muted-foreground bg-muted"
            >
              {currencySymbol}
            </Badge>
          )}
          <div
            className={cn(
              ...
              isHero
                ? TYPOGRAPHY_CLASSNAMES.text3xlSemiBold
          ...
          <div className="flex items-center justify-between">
            ...
            <ChevronRight
              className={cn(
                ...
                href &&
                  'group-hover/kpi-card:translate-x-0.5 group-hover/kpi-card:text-primary'
              )}
            />
          </div>
