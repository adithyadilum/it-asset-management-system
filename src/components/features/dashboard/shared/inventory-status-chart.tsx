...
const STATUS_METRICS_META: Record<
  string,
  {
    metricLabel: string;
    insight: string;
...
> = {
  Assigned: {
    ...
    insight:
      'Assets currently in active use by employees. Target: >75% for optimal device deployment.',
    badgeClass:
  ...
  'New / Available': {
    ...
    insight:
      'Unassigned stock ready for immediate onboarding or hot-swaps. Recommended buffer: 5-10%.',
    badgeClass:
  ...
  'In Repair': {
    ...
    insight:
      'Temporarily out of service for servicing. Goal: Keep turnaround under 5 business days.',
    badgeClass:
  ...
  Defective: {
    ...
    insight:
      'Requires technician diagnostics to repair or move to decommissioning queue.',
    badgeClass:
      'bg-destructive/10 text-red-500 border-red-500/20 dark:bg-destructive/20',
    type: 'active',
  },
  Lost: {
    metricLabel: 'Shrinkage Risk',
    insight:
      'Unaccounted assets. High lost rates indicate a need for stricter physical audit logging.',
    badgeClass:
      'bg-orange-500/10 text-orange-500 border-orange-500/20 dark:bg-orange-500/20',
    type: 'active',
  },
  Retired: {
    metricLabel: 'End of Life',
    insight:
      'Asset reached end of its logical lifetime. Safe to store or prepare for physical disposal.',
    badgeClass:
      'bg-muted0/10 text-muted-foreground border-slate-500/20 dark:bg-muted0/20',
    type: 'inactive',
  },
  'Pending Disposal': {
    metricLabel: 'Decommissioning Queue',
    insight:
      'Awaiting final financial/compliance write-off and environmental disposal sign-off.',
    badgeClass:
  ...
  Disposed: {
    ...
    insight:
      'Permanently removed from active books. Eco-friendly recycled or sold. Records kept for audit.',
    badgeClass:
            ...
            <div className="h-full w-[55%] min-h-60 relative flex items-center justify-center">
              ...
              >
                <PieChart
                  ...
                  aria-label="Current inventory status donut chart"
                >
                  <Pie
                    data={inventoryData}
                    dataKey="value"
                  ...
                  <div className="bg-popover/98 backdrop-blur-md border border-border shadow-2xl rounded-xl p-3 w-48 flex flex-col gap-1.5 text-popover-foreground">
                    ...
                    <div className="flex items-center gap-1.5 pb-1 border-b border-border/50">
                      <div
                        ...
                        style={{
                          backgroundColor: inventoryData[hoveredIndex].color,
                        }}
                      />
                      <span className="font-bold text-[10px] text-foreground tracking-tight truncate">
                        {inventoryData[hoveredIndex].name}
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-1 text-[8.5px] leading-tight">
                      <div className="flex flex-col bg-muted/40 p-1 rounded-md border border-border/30">
                        ...
                        <span className="text-[10px] font-bold text-foreground mt-0.5">
                          {inventoryData[hoveredIndex].value}{' '}
                          <span className="text-[7.5px] font-normal text-muted-foreground">
                            unit
                            {inventoryData[hoveredIndex].value !== 1 ? 's' : ''}
                          </span>
                        </span>
                      </div>
                      <div className="flex flex-col bg-muted/40 p-1 rounded-md border border-border/30">
                        ...
                        <span className="text-[10px] font-bold text-foreground mt-0.5">
                          {(
                            (inventoryData[hoveredIndex].value / totalAll) *
                            100
                          ).toFixed(1)}
                          %
                        </span>
                    ...
                    <div className="bg-muted/50 p-1.5 rounded-lg border border-border/30 text-[8.5px] leading-relaxed text-muted-foreground/90">
                      <p>
                        {
                          STATUS_METRICS_META[inventoryData[hoveredIndex].name]
                            ?.insight
                        }
                      </p>
                    </div>
              ...
              {inventoryData.map((entry, i) => (
                ...
                >
                  ...
                  <span
                    className={cn(
                      TYPOGRAPHY_CLASSNAMES.textXsRegular,
                      'text-muted-foreground leading-none truncate'
                    )}
                  >
                    {entry.name}
                  </span>
