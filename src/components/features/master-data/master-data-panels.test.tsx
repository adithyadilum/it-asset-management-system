...
vi.mock('@/components/features/master-data/master-data-create-panel', () => ({
  MasterDataCreatePanel: (props: any) => (
    <div data-testid="create-panel" data-open={props.isOpen} data-entity={props.entity}>
      Create Panel
    </div>
  )
}));
...
vi.mock('@/components/features/master-data/master-data-record-panel', () => ({
  MasterDataRecordPanel: (props: any) => (
    <div data-testid="record-panel" data-open={props.isOpen} data-entity={props.entity} data-id={props.recordId}>
      Record Panel
    </div>
  )
}));
...
describe('MasterDataPanels', () => {
  ...
  it('renders create panel when currentPanel is create', () => {
    render(<MasterDataPanels {...defaultProps} currentPanel="create" entity="brands" />);
  ...
  it('renders record panel when currentPanel is record', () => {
    render(<MasterDataPanels {...defaultProps} currentPanel="record" entity="brands" recordId="123" />);
  ...
  it('renders both as closed when currentPanel is undefined', () => {
    ...
    expect(screen.getByTestId('create-panel')).toHaveAttribute('data-open', 'false');
    expect(screen.getByTestId('record-panel')).toHaveAttribute('data-open', 'false');
  });
});
