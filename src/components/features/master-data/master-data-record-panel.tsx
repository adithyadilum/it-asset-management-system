...
export function MasterDataRecordPanel({
  ...
  const handleConfirmDelete = useCallback(async () => {
    ...
    try {
      const result = await deleteMasterDataRecords(normalizedEntity, [
        selectedRecord.id,
      ]);
      if (result.success) {
        tiqriToast.success(result.message);
      ...
      case 'locations':
        return (
          <LocationForm
            key={formKey}
            {...commonProps}
            initialData={selectedRecord as unknown as MasterDataLocationRow}
            locations={locations}
          />
        );
      case 'asset-categories':
        return (
          <CategoryForm
            key={formKey}
            {...commonProps}
            initialData={selectedRecord as unknown as MasterDataCategoryRow}
          />
        );
      case 'brands':
        return (
          <BrandForm
            key={formKey}
            {...commonProps}
            initialData={selectedRecord as unknown as MasterDataBrandRow}
          />
        );
      case 'device-models':
        return (
          <DeviceModelForm
            ...
            {...commonProps}
            ref={customFormRef}
            initialData={selectedRecord as unknown as MasterDataDeviceModelRow}
            brands={brands}
      ...
      case 'vendors':
        return (
          <VendorForm
            key={formKey}
            {...commonProps}
            initialData={selectedRecord as unknown as MasterDataVendorRow}
          />
        );
      case 'owners':
        return (
          <OwnerForm
            key={formKey}
            {...commonProps}
            initialData={selectedRecord as unknown as MasterDataOwnerRow}
          />
        );
      case 'departments':
        return (
          <DepartmentForm
            key={formKey}
            {...commonProps}
            initialData={selectedRecord as unknown as MasterDataDepartmentRow}
          />
        );
      case 'statuses':
        return (
          <StatusForm
            key={formKey}
            {...commonProps}
            initialData={selectedRecord as unknown as MasterDataCustomStatusRow}
          />
        );
    }
  };
          ...
          >
            ...
            {!isDetailMode && selectedRecord && (
              <div className="mt-8 rounded-md border border-red-200 p-4 dark:border-red-900/50 bg-red-50/50 dark:bg-red-900/10">
                <h3
                  className={`${TYPOGRAPHY_CLASSNAMES.textSmSemiBold} text-red-600 dark:text-red-400 mb-1`}
                >
                  Danger Zone
                </h3>
                <p
                  className={`${TYPOGRAPHY_CLASSNAMES.textSmRegular} text-muted-foreground mb-4`}
                >
                  {linkedAssetsCount > 0
