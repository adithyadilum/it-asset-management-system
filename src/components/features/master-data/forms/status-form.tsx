...
export function StatusForm({
    ...
    const [iconName, setIconName] = useState(initialData?.iconName || "CircleDot");
    ...
    useEffect(() => {
        ...
        const dirty =
            ...
            JSON.stringify([...allowedActions].sort()) !== JSON.stringify([...(initialData.allowedActions || [])].sort());
        onDirtyStateChange?.(dirty);
    }, [name, iconName, colorTheme, isActive, allowedActions, initialData, onDirtyStateChange]);
    ...
    return (
        <>
            ...
            {isEdit && initialData && (
                <RecordIdPreview entity="statuses" record={initialData as unknown as Record<string, unknown>} numericRecordId={initialData.id} />
            )}
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                <div className="space-y-2">
                    ...
                    {isDetailMode ? (
                        <div className={`flex items-center gap-2 ${READ_ONLY_INPUT_CLASSNAME} rounded-md px-3`}>
                ...
                <div className="space-y-2">
                    ...
                    {isDetailMode ? (
                        <div className={`flex items-center gap-2 ${READ_ONLY_INPUT_CLASSNAME} rounded-md px-3`}>
                            <div className={`h-4 w-4 rounded-full border ${STATUS_THEMES[colorTheme]}`} />
                            <span>{STATUS_COLORS.find(c => c.value === colorTheme)?.label || colorTheme}</span>
                        </div>
                    ) : (
                        <Select value={colorTheme} onValueChange={(val) => setColorTheme(val as StatusTheme)}>
                            <SelectTrigger className="h-10">
                                <div className="flex items-center gap-2">
                                    <div className={`h-4 w-4 rounded-full border ${STATUS_THEMES[colorTheme]}`} />
                            ...
                            <SelectContent>
                                {STATUS_COLORS.map((theme) => (
                                    <SelectItem key={theme.value} value={theme.value}>
                                        <div className="flex items-center gap-2">
                                            <div className={`h-4 w-4 rounded-full border ${STATUS_THEMES[theme.value]}`} />
                            ...
                            <Checkbox
                                ...
                                onCheckedChange={(checked) => {
                                    ...
                                    if (checked) {
                                        ...
                                        setAllowedActions((prev) => prev.filter((a) => a !== action.id));
                                    }
                                }}
