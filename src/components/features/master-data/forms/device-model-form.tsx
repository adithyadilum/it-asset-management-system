...
export const DeviceModelForm = forwardRef<
  MasterDataFormRef,
...
>(
  (
    ...
    const [brandId, setBrandId] = useState(
      initialData?.brandId ? String(initialData.brandId) : ''
    );
    ...
    const [modelSpecValues, setModelSpecValues] = useState<
      Record<string, string>
    ...
    const activeCategoriesForModel = useMemo(
      () =>
        categories.filter(
          (category) =>
            (isEdit ? true : category.isActive) && category.pillar === pillar
        ),
    ...
    const displayModelImageUrl =
      modelImagePreviewUrl ??
      (removeExistingImage ? '' : selectedModelImageUrl);
    ...
    useEffect(() => {
      ...
      const dirty =
        ...
        JSON.stringify(modelSpecValues) !==
          JSON.stringify(initialModelSpecValues) ||
        modelImageFile !== null ||
        removeExistingImage;
    ...
    const handleModelImageSelection = useCallback(
      (files: FileList | null) => {
        const selectedFile = files?.[0] ?? null;

        if (selectedFile && !isModelImageFile(selectedFile)) {
          tiqriToast.error(
            'Upload a valid image file (PNG, JPG, JPEG, WEBP, GIF, BMP, SVG, or AVIF).'
          );
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div className="space-y-2">
            ...
            {isDetailMode ? (
              <input
                className={
                  READ_ONLY_INPUT_CLASSNAME + ' w-full rounded-md px-3'
                }
                value={
                  brands.find((b) => String(b.id) === brandId)?.name || 'N/A'
                }
                readOnly
              />
            ) : (
                  ...
                  return (
                    <div key={spec.fieldName} className="space-y-2">
                      ...
                      <div className="flex h-9 items-center px-1">
                        {isDetailMode ? (
                          <span
                            className={`${TYPOGRAPHY_CLASSNAMES.textSmMedium} ${specValue === 'true' ? 'text-green-600' : 'text-muted-foreground'}`}
                          >
                  ...
                  <div key={spec.fieldName} className="space-y-2">
                    ...
                    {isDetailMode ? (
                      ...
                      <Input
                        ...
                        value={specValue}
                        onChange={(e) =>
                          updateModelSpecValue(spec.fieldName, e.target.value)
                        }
        ...
        <div className="space-y-3 border-t pt-4">
          ...
          <div className="space-y-4">
            {(showModelImageUploader ||
              (!hasSelectedModelImage && !isDetailMode)) && (
              <div
                onDragOver={(e) => {
                  e.preventDefault();
