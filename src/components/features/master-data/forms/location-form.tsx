...
export function LocationForm({
    ...
    const [type, setType] = useState<LocationType | "">((initialData?.type as LocationType) || "");
    ...
    return (
        <>
            ...
            {isEdit && initialData && (
                <RecordIdPreview entity="locations" record={initialData as unknown as Record<string, unknown>} numericRecordId={initialData.id} />
            )}
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div className="space-y-2">
                    ...
                    {isDetailMode ? (
                        <input className={READ_ONLY_INPUT_CLASSNAME + " w-full rounded-md px-3"} value={type} readOnly />
                    ) : (
                        <SearchableDropdown
                    ...
                    {fieldError("type") && (
                        <p className={`${TYPOGRAPHY_CLASSNAMES.textSmRegular} text-red-600`}>{fieldError("type")}</p>
                    )}
                </div>
            </div>
            ...
            <div className="space-y-2">
                ...
                {isDetailMode ? (
                    <input className={READ_ONLY_INPUT_CLASSNAME + " w-full rounded-md px-3"} value={parentLocationLabel} readOnly />
                ) : (
                    <SearchableDropdown
                        options={[
                ...
                {fieldError("parentId") && (
                    <p className={`${TYPOGRAPHY_CLASSNAMES.textSmRegular} text-red-600`}>{fieldError("parentId")}</p>
                )}
            </div>
