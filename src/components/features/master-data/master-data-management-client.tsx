...
export function MasterDataManagementClient({
  ...
  const [activeTab, setActiveTab] = useState<MasterDataTabId>(
    initialTab ?? 'asset-categories'
  );
  const [searchByTab, setSearchByTab] =
    useState<Record<MasterDataTabId, string>>(EMPTY_SEARCH_STATE);
  ...
  const [pendingDeleteEntity, setPendingDeleteEntity] =
    useState<MasterDataTabId | null>(null);
  const [pendingDeleteRows, setPendingDeleteRows] = useState<
    Array<{ id: number; linkedAssets: number }>
  >([]);

  const handleDeleteClick = useCallback(
    (
      entity: MasterDataTabId,
      selectedRows: Array<{ id: number; linkedAssets: number }>
    ) => {
  ...
  const getDeleteItemsData = useCallback(() => {
    ...
    const getItemName = (entity: MasterDataTabId, item: DataItem): string => {
      if (entity === 'vendors' || entity === 'owners') {
        return (
          (item as MasterDataVendorRow | MasterDataOwnerRow).companyName ||
          'N/A'
        );
      }
      return (
        (item as Exclude<DataItem, MasterDataVendorRow | MasterDataOwnerRow>)
          .name || 'N/A'
      );
    };
  ...
  const blockedDeleteCodeIds = useMemo(() => {
    ...
    const resolveBlockedCodes = <T extends { id: number; code: string | null }>(
      rows: T[]
    ) =>
      rows
        .filter((row) => blockedRowIds.has(row.id))
        .map((row) =>
          resolveMasterDataCode(pendingDeleteEntity, row.code, row.id)
        );
  ...
  const categoryColumns = useMemo<ColumnDef<MasterDataCategoryRow>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <StatusBadge
            variant="linkedAssets"
            count={row.original.linkedAssets}
          />
        ),
      },
    ],
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <span className="text-xs text-muted-foreground">
            {row.original.allowedActions?.length
              ? row.original.allowedActions.join(', ')
              : 'None'}
          </span>
        ),
      },
  ...
  const filteredBrands = useMemo(
    () =>
      brands.filter((item) =>
        containsSearch(
          [item.id, item.code, item.name, item.linkedAssets],
          searchByTab.brands
        )
      ),
  ...
  const buildMasterDataUrl = useCallback(
    (
      overrides: Partial<
        Record<
          'tab' | 'panel' | 'entity' | 'id' | 'mode' | 'animate',
          string | undefined
        >
      >
    ) => {
        ...
        <div className="mt-4 flex min-h-0 flex-1 flex-col gap-4">
              ...
              {showTypeFilter && (
                <div className="flex items-center gap-2">
                  ...
                  <Select
                    value={pillarType}
                    onValueChange={(value) =>
                      setPillarType(value as PillarFilter)
                    }
                  >
          ...
          >
            <DataTable
              ...
              isRowActive={(row) =>
                Boolean(activeRecordId && row.id === activeRecordId)
              }
