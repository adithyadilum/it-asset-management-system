...
describe('MultiAssetAssignmentModal', () => {
  ...
  const mockAssets = [
    { assetId: '1', assetTag: 'TAG-1', assetName: 'Laptop 1', assetGroup: 'Hardware' },
    { assetId: '2', assetTag: 'TAG-2', assetName: 'Laptop 2', assetGroup: 'Hardware' },
  ];
  ...
  it('submits bulk assignment correctly', async () => {
    (bulkAssignAssetsAction as any).mockResolvedValue({ success: true, count: 2 });
    ...
    fireEvent.change(notesTextarea, { target: { value: 'Bulk assignment test' } });
  ...
  it('prevents user assignment if assets include furniture', () => {
    renderModal({
      assets: [
        ...mockAssets,
        { assetId: '3', assetTag: 'TAG-3', assetName: 'Chair', assetGroup: 'Office Furniture' }
      ]
    });
