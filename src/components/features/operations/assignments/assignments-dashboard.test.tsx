...
vi.mock('./multi-asset-assignment-modal', () => ({
  MultiAssetAssignmentModal: ({ isOpen }: any) =>
    isOpen ? (
      <div data-testid="multi-asset-assignment-modal">Multi Modal Open</div>
    ) : null,
}));
...
vi.mock('./process-return-modal', () => ({
  ProcessReturnModal: ({ isOpen }: any) =>
    isOpen ? (
      <div data-testid="process-return-modal">Return Modal Open</div>
    ) : null,
}));
  ...
  const renderDashboard = () => {
    return render(
      <CurrencyProvider initialCurrency="USD">
        <AssignmentsDashboard data={mockData} />
      </CurrencyProvider>
    );
  };
