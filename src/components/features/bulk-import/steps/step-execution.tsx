...
export function StepExecution({ state, dispatch, onDone }: StepExecutionProps) {
      ...
      try {
        ...
        if (result.success && result.summary) {
          ...
          if (result.summary.failedCount > 0) {
            tiqriToast.warning(
              `Import completed with ${result.summary.failedCount} failures.`
            );
          } else {
            tiqriToast.success(
              `${result.summary.successCount} assets imported successfully.`
            );
          }
        } else {
  ...
  const handleDownloadErrors = () => {
    ...
    const blob = new Blob([state.executionResult.errorCsvData], {
  ...
  const { isExecuting, executionProgress, executionResult, previewResult } =
    state;
          ...
          <div className="flex flex-col items-center justify-center h-full gap-6 py-10 animate-in fade-in duration-500">
            <div className="flex flex-col items-center gap-4 w-full max-w-md">
              ...
              <p className="text-lg font-semibold animate-pulse">
                Importing {totalToImport}{' '}
                {totalToImport === 1 ? 'asset' : 'assets'}...
              </p>
              <Progress
            ...
            <p className="text-sm text-muted-foreground text-center max-w-sm mt-4">
              Please do not close this window. Row-level transactions are being
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  >
                    ...
                    {executionResult.successCount === 1 ? 'asset' : 'assets'}{' '}
                  ...
                  {executionResult.failedCount > 0 && (
                    <p className="text-sm font-medium text-destructive mt-1 flex items-center gap-1.5">
                      ...
                      {executionResult.failedCount === 1 ? 'row' : 'rows'}{' '}
              ...
              <div className="flex items-center justify-between p-4 rounded-xl border bg-card shadow-sm">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    Download a CSV of rows that failed to insert
                  </p>
                </div>
                <Button
                  type="button"
              ...
              <div className="rounded-xl border bg-card overflow-hidden flex flex-col">
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {showAllTags
                        ? 'Showing all generated tags'
                        : `Showing first ${Math.min(10, executionResult.importedAssetTags.length)} tags`}
                    </p>
                  </div>
                  <Button
                    variant="ghost"
                ...
                <ScrollArea
                  className={cn(
                    ...
                    showAllTags ? 'h-50' : 'max-h-35'
                  )}
                >
                  <div className="p-4 flex flex-wrap gap-2">
                    {(showAllTags
                      ? executionResult.importedAssetTags
                      : executionResult.importedAssetTags.slice(0, 10)
                    ).map((tag) => (
                      <span
                        key={tag}
                    ...
                    {!showAllTags &&
                      executionResult.importedAssetTags.length > 10 && (
