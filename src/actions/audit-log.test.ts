...
describe('audit-log server actions', () => {
  ...
  describe('resolveAuditValueLabels', () => {
    it('throws error for unauthorized users', async () => {
      ...
      await expect(resolveAuditValueLabels([])).rejects.toThrow(
        'Unauthorized access to audit metadata.'
      );
    });
    ...
    it('resolves labels successfully for admin', async () => {
      ...
      const records = [
        {
          oldValue: { assetId: 'uuid-1', locationId: 1 },
          newValue: { assetId: 'uuid-2', locationId: 2 },
        },
      ];
      ...
      mockDb.select.mockReturnValueOnce(
        chain([
          { id: 'uuid-1', assetTag: 'TAG1', name: 'Asset1' },
          { id: 'uuid-2', assetTag: 'TAG2', name: 'Asset2' },
        ])
      ); // assets
      mockDb.select.mockReturnValueOnce(
        chain([
          { id: 1, code: 'LOC1', name: 'Location1' },
          { id: 2, code: 'LOC2', name: 'Location2' },
        ])
      ); // locations
  ...
  describe('resolveTargetEntityLabels', () => {
    it('resolves labels grouping by entityType', async () => {
      ...
      mockDb.select.mockReturnValueOnce(
        chain([{ id: 'user-1', name: 'John Doe', email: 'john@example.com' }])
      ); // users
  ...
  describe('getAuditLogs', () => {
    it('throws error for unauthorized users', async () => {
      ...
      await expect(getAuditLogs({ page: 1, pageSize: 10 })).rejects.toThrow(
  ...
  describe('getAssetAuditHistory', () => {
    it('throws error for unauthorized users', async () => {
      mockGetAuthenticatedUser.mockResolvedValue(EMPLOYEE_USER);
      await expect(getAssetAuditHistory('uuid-1', 1, 10)).rejects.toThrow(
