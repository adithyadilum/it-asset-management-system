...
export async function getAdminMobileMetrics(): Promise<AdminMobileMetrics> {
  ...
  const [assignedAssetsResult, pendingApprovalsResult, recentActivitiesResult] = await Promise.all([
  ...
  const recentActivities: RecentActivity[] = recentActivitiesResult.map((log) => ({
    id: log.id,
    action: log.action,
