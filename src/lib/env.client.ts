...
const clientEnvSchema = z.object({
  ...
  NEXT_PUBLIC_NOTIFICATION_POLL_INTERVAL: z.coerce
    .number()
    .int()
    .positive()
    .default(30000),
...
const result = clientEnvSchema.safeParse({
  ...
  NEXT_PUBLIC_NOTIFICATION_POLL_INTERVAL:
...
if (!result.success) {
  ...
  throw new Error(
    'Invalid client environment variables. Fix them in .env or .env.local'
  );
}
