...
export const createWebhookSchema = z.object({
  ...
  url: z.string().url().refine((v) => v.startsWith('https://'), { message: 'URL must use HTTPS' }),
  events: z.array(z.enum([...WEBHOOK_EVENT_TYPES] as [string, ...string[]])).min(1, 'Select at least one event'),
});

export const updateWebhookSchema = z.object({
  name: z.string().min(3).max(100).optional(),
  url: z.string().url().optional().refine((v) => !v || v.startsWith('https://'), { message: 'URL must use HTTPS' }),
