...
describe('Financial Math Utility', () => {
  ...
  describe('calculateStraightLineDepreciation', () => {
    it('returns 0 when original price is 0 or negative', () => {
      expect(calculateStraightLineDepreciation(0, 60, '2023-01-15')).toBe(0);
      expect(calculateStraightLineDepreciation(-100, 60, '2023-01-15')).toBe(0);
    });

    it('returns original price when purchaseDate is missing or invalid', () => {
      expect(calculateStraightLineDepreciation(1000, 60, null)).toBe(1000);
      expect(calculateStraightLineDepreciation(1000, 60, 'invalid')).toBe(1000);
    });

    it('returns original price when elapsed months is 0 or negative', () => {
      expect(calculateStraightLineDepreciation(1000, 60, '2024-01-01')).toBe(1000); // 0 elapsed
      expect(calculateStraightLineDepreciation(1000, 60, '2024-02-01')).toBe(1000); // negative elapsed
    });
    ...
    it('calculates correct depreciation after 1 year with 5 year default life (60 months)', () => {
      ...
      expect(calculateStraightLineDepreciation(1000, null, '2023-01-15')).toBe(800);
    });
    ...
    it('calculates correct depreciation after 1 year with custom 3 year life (36 months)', () => {
      ...
      expect(calculateStraightLineDepreciation(1000, 36, '2023-01-15')).toBeCloseTo(666.67, 1);
    });

    it('floors to 0 when asset is older than useful life', () => {
      // 1000 base. 60 month life. 61 months elapsed (2018-12-15 -> 2024-01-15)
      expect(calculateStraightLineDepreciation(1000, 60, '2018-12-15')).toBe(0);
    });
  });
});
