...
_Note: While not explicitly declared in `schema.ts`, these indexes should be provisioned at the PostgreSQL database level for scalability:_
...
| `'URL'`              | _None (Middleware level)_                      | Endpoint access logged by routing or security middleware          |

## 6. JSONB Payload Examples

### 6.1 Asset Status Change (`UPDATE`)

_Notice the delta diff mechanism: only the modified properties are stored in `oldValue` and `newValue`._
...
_On creation, `oldValue` is `null` and the complete state is stored in `newValue`._
...
_Captures blocked actions, logging the target resource and actor information for immediate forensic traceability._
