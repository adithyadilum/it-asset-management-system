...
### 1.5 SSO Login — Azure AD Sign-In

**Screen:** `/login` (Unauthenticated Landing Page)

**Layout Description:**
The corporate SSO entry point. Users authenticate exclusively via Microsoft Azure AD — no local username/password fields exist.

| Zone           | Content                                                                                                                              |
| :------------- | :----------------------------------------------------------------------------------------------------------------------------------- |
| **Branding**   | IDAMS logo, system title "IT Asset Management System", and TIQRI corporate branding.                                                 |
| **Action**     | "Sign in with Microsoft" primary button. Triggers OAuth 2.0 / OIDC Authorization Code Flow via Azure AD with MFA enforcement.        |
| **Post-Login** | First-time users auto-mapped to "Standard Employee" role. RBAC middleware loads permissions and routes to the appropriate dashboard. |

**Wireframe Reference:**

![SSO Login — Azure AD Sign-In](../../1_requirements/user-story-specs/images/Login%20-%20Desktop.png)

**Linked Stories:** US-1.1.1

---

### 1.6 API Key Management — Integrations

**Screen:** Settings → Integrations → API Keys

**Layout Description:**
A management interface for creating, viewing, and revoking programmatic API keys used by external systems (e.g., HR platforms, monitoring tools).

| Zone               | Content                                                                                                                                                                                              |
| :----------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Toolbar**        | "Generate New API Key" button.                                                                                                                                                                       |
| **Data Grid**      | Existing keys listed with columns: Key Name (e.g., "Workday HRIS"), Created Date, Last Used, Status (Active / Revoked), Actions (Revoke).                                                            |
| **Generate Modal** | Modal dialog: Key Name text input + "Generate" button. On confirm: the secret key is displayed **exactly once** with a copy-to-clipboard action. Success state prominently shows the full key value. |
| **Revocation**     | "Revoke" action per row. Revoked keys immediately return `401 Unauthorized` on future requests.                                                                                                      |

**Wireframe References:**

![API Key Management — Settings](../../1_requirements/user-story-specs/images/Integrations%20API%20-%20Desktop.png)
![API Key Generation — Modal](../../1_requirements/user-story-specs/images/Integrations%20API%20Key%20gen%20modal%20-%20Desktop.png)
![API Key Generation — Success State](../../1_requirements/user-story-specs/images/Integrations%20API%20Key%20gen%20modal%20success%20-%20Desktop.png)

**Linked Stories:** US-1.5.1, US-1.5.2, US-1.5.3

---

### 1.7 Webhook Configuration — Integrations

**Screen:** Settings → Integrations → Webhooks

**Layout Description:**
A configuration interface for registering external webhook URLs that receive real-time push notifications when specific system events occur (e.g., Asset Disposed, Asset Assigned).

| Zone                | Content                                                                                                                                                                               |
| :------------------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Data Grid**       | Registered webhooks listed with columns: Event Type, Target URL, Status (Active / Disabled), Last Triggered, Actions (Edit, Delete).                                                  |
| **Configure Modal** | Dialog form: Event Type dropdown (Asset Created, Asset Assigned, Asset Disposed, Status Changed, etc.), Target URL text input, optional Secret Token for HMAC signature verification. |
| **Delivery**        | Backend dispatches HTTP POST payloads asynchronously with retry logic for failed deliveries. Subscriptions stored in `WebhookSubscriptions` table.                                    |

**Wireframe References:**

![Webhook Configuration — Settings](../../1_requirements/user-story-specs/images/Integrations%20Webhooks%20-%20Desktop.png)
![Webhook Configuration — Modal](../../1_requirements/user-story-specs/images/Integrations%20Webhooks%20Configure%20modal%20-%20Desktop.png)
...
![QR Code Preview & Print](<../../1_requirements/user-story-specs/images/Asset%20Details%20(QR%20preview)-%20Desktop.png>)

**Linked Stories:** US-2.3.1, US-2.3.2

### 2.6 Bulk CSV/Excel Import

**Screen:** Assets → Bulk Import

**Layout Description:**
A dedicated import wizard for mass-populating the asset registry from CSV or Excel (.xlsx) files during legacy migration or bulk batch purchases.

| Zone               | Content                                                                                                                                                                                                   |
| :----------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Upload Zone**    | Drag-and-drop area accepting `.csv` and `.xlsx` files. Displays filename and size on drop.                                                                                                                |
| **Column Mapping** | Interactive mapping UI: each spreadsheet column is paired to an IDAMS field via dropdowns. Auto-mapping suggested where column headers match system fields.                                               |
| **Validation**     | Preview grid showing parsed rows with inline validation indicators. Invalid rows highlighted with error descriptions (e.g., duplicate serial number, missing required field).                             |
| **Import Action**  | Supports **Partial Success** — valid rows import successfully while failed rows generate a **downloadable error report** (CSV). Auto-generates Asset IDs and QR URLs for every successfully imported row. |

**Wireframe Reference:**

![Bulk CSV/Excel Import](../../1_requirements/user-story-specs/images/Bulk%20Asset%20Registry%20-%20Desktop.png)

**Linked Stories:** US-2.1.5

---

### 2.7 Mobile Scanner & Lookup

**Screen:** PWA Mobile Interface — `assets.tiqri.com/scan`

**Layout Description:**
A mobile-first Progressive Web App providing real-time QR/barcode scanning and instant asset lookup without requiring a native app install.

| Zone             | Content                                                                                                                                                                                                      |
| :--------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Admin Portal** | Mobile-optimised landing with quick-access scanner button and recent scan history cards.                                                                                                                     |
| **Scanner View** | Full-screen camera overlay with a centred targeting reticle. "Flashlight" toggle button. Permission prompts for camera access. Supports QR (2D) and 1D barcodes (Code 128, UPC, EAN).                        |
| **Scan Success** | On successful decode: device vibrates briefly (haptic feedback via `navigator.vibrate`). Bottom-sheet slides up displaying: Asset ID, Model, Custodian, Status, and a "Flag for Repair" quick-action button. |

**Wireframe References:**

![Mobile Admin Portal](../../1_requirements/user-story-specs/images/Admin%20Portal%20-%20Mobile.png)
![Mobile QR Scanner Interface](../../1_requirements/user-story-specs/images/Scanner%20-%20Mobile.png)
![Mobile Scan Success — Bottom-Sheet](../../1_requirements/user-story-specs/images/Scan%20success%20-%20Mobile.png)

**Linked Stories:** US-2.4.1, US-2.4.2

---

### 2.8 Mobile Empty State Fallbacks

**Screen:** PWA Mobile → Desktop-Only Feature Accessed

**Layout Description:**
Graceful fallback screens displayed when a mobile user navigates to a URL designed exclusively for desktop viewports (e.g., `/financials`, `/registry`).

| Zone             | Content                                                                                                                                             |
| :--------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Illustration** | Friendly illustration card with a "Desktop Required" visual and message: "This feature is optimised for desktop. Please switch to a larger screen." |
| **Action**       | "Open Scanner" redirect button routing the user back to the mobile-supported scanner interface.                                                     |
| **Detection**    | CSS Media Queries or React viewport hooks detect mobile screen widths and trigger the fallback.                                                     |

**Wireframe Reference:**

![Mobile Empty State Fallback](../../1_requirements/user-story-specs/images/Admin%20Portal%20-%20Empty%20-%20Mobile.png)
...
![Employee Portal — Mobile View](../../1_requirements/user-story-specs/images/Employee%20Portal%20-%20Mobile.png)
...
**Screen:** Asset Details → History Tab (Full-Page / Slide-Out View)

**Layout Description:**
A comprehensive chronological timeline displaying the complete chain of custody and lifecycle events for a specific asset since purchase.

| Zone            | Content                                                                                                                                                                |
| :-------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Timeline**    | Vertical timeline UI component. Each entry: Timestamp, Action Type (Assigned, Returned, Status Changed, Repaired, Edited), Old Value → New Value, Actor (name + role). |
| **Data Source** | Backend query fetches asset-specific events from the global System Audit Log and formats them chronologically.                                                         |
| **Export**      | "Download History as CSV" button at the footer exports this specific asset's timeline for auditing purposes.                                                           |
| **Integration** | Embedded within the Asset Details Slide-Out Panel's History tab (§2.4) and also available as a standalone expanded view.                                               |
...
### 3.9 Bulk Location Transfer

**Screen:** Asset Registry → Bulk Select → Bulk Transfer Modal

**Layout Description:**
A modal dialog for updating the physical location of multiple assets simultaneously, triggered from the registry grid's bulk-action toolbar.

| Zone                | Content                                                                                                                                                                                     |
| :------------------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Selected Assets** | Summary list of selected Asset IDs and Names with total count prominently displayed.                                                                                                        |
| **Target Location** | Searchable Location dropdown (Building > Floor > Room) from Master Data.                                                                                                                    |
| **Footer**          | "Cancel" and "Confirm Transfer" buttons. On confirm: all selected assets updated to the target location in a single database transaction. Each individual change recorded in the Audit Log. |
...
| 45  | `Login - Desktop.png`                                  | 1    | SSO Login — Azure AD Sign-In                 |
| 46  | `Integrations API - Desktop.png`                       | 1    | API Key Management — Settings                |
| 47  | `Integrations API Key gen modal - Desktop.png`         | 1    | API Key Generation — Modal                   |
| 48  | `Integrations API Key gen modal success - Desktop.png` | 1    | API Key Generation — Success State           |
| 49  | `Integrations Webhooks - Desktop.png`                  | 1    | Webhook Configuration — Settings             |
| 50  | `Integrations Webhooks Configure modal - Desktop.png`  | 1    | Webhook Configuration — Modal                |
| 51  | `Bulk Asset Registry - Desktop.png`                    | 2    | Bulk CSV/Excel Import                        |
| 52  | `Scanner - Mobile.png`                                 | 2    | Mobile QR Scanner Interface                  |
| 53  | `Scan success - Mobile.png`                            | 2    | Mobile Scan Success — Bottom-Sheet           |
| 54  | `Admin Portal - Mobile.png`                            | 2    | Mobile Admin Portal                          |
| 55  | `Admin Portal - Empty - Mobile.png`                    | 2    | Mobile Empty State Fallback                  |
| 56  | `Employee Portal - Mobile.png`                         | 3    | Employee Portal — Mobile View (Alt)          |
| 57  | `Asset History - Desktop.png`                          | 3    | Asset Chain of Custody — History Timeline    |
